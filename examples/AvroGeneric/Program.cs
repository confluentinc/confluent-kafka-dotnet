...
namespace Confluent.Kafka.Examples.AvroGeneric
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var keySerializer = new ConfluentAvroSerializer<User>(schemaRegistry, true);
            var valueSerializer = new ConfluentAvroSerializer<User>(schemaRegistry, false);
            var deserializer = new ConfluentAvroDeserializer<User>(schemaRegistry);

            using (var consumer = new Consumer<User, User>(consumerConfig, deserializer, deserializer))
            using (var producer = new Producer<User, User>(producerConfig, keySerializer, valueSerializer))
            {
                ...
                while ((text = Console.ReadLine()) != "q")
                {
                    ...
                    var deliveryReport = producer.ProduceAsync(topicName, user, user);
