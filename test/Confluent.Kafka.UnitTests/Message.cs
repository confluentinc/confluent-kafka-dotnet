...
namespace Confluent.Kafka.Tests
{
    public class MessageTests
    {
        ...
        public void ConstuctorAndProps()
        {
            byte[] key = new byte[0];
            byte[] val = new byte[0];
            var timestamp = Timestamp.DateTimeToUnixTimestampMs(new DateTime(2001, 3, 4));
            var mi = new Message("tp1", 24, 33, key, val, new Timestamp(timestamp, TimestampType.CreateTime), new Error(ErrorCode.NoError));
            ...
            Assert.Equal(mi.Timestamp, new Timestamp(timestamp, TimestampType.CreateTime));
        ...
        {
            var timestamp = Timestamp.DateTimeToUnixTimestampMs(new DateTime(2001, 3, 4));
            var mi = new Message<string, string>("tp1", 24, 33, "mykey", "myval", new Timestamp(timestamp, TimestampType.CreateTime), new Error(ErrorCode.NoError));
