...
namespace Confluent.Kafka.UnitTests.Serialization
{
    public class FloatTests
    {
        ...
        public void CanReconstructFloat()
        {
            ...
            {
                Assert.Equal(value, Deserializers.Float(Serializers.Float(value), false));
            }
        }
        ...
        {
            Assert.ThrowsAny<ArgumentNullException>(() => Deserializers.Float(null, true));
        }

        [Fact]
        public void DeserializeArgLengthNotEqual4Throw()
        {
            Assert.ThrowsAny<ArgumentException>(() => Deserializers.Float(new byte[0], false));
            Assert.ThrowsAny<ArgumentException>(() => Deserializers.Float(new byte[3], false));
            Assert.ThrowsAny<ArgumentException>(() => Deserializers.Float(new byte[5], false));
        }
