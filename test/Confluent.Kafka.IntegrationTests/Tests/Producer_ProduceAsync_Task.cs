...
namespace Confluent.Kafka.IntegrationTests
{
    ...
    public partial class Tests
    {
        ...
        public void Producer_ProduceAsync_Task_WithInstrumentation(string bootstrapServers)
        {
            Producer_ProduceAsync_Task(bootstrapServers, true);
        }

        [Theory, MemberData(nameof(KafkaParameters))]
        public void Producer_ProduceAsync_Task(string bootstrapServers, bool instrument = false)
        {
            ...
            try
            {
                ...
                if (instrument)
                {
                    ...
                    Assert.Equal(partitionedTopic, EventObserverAndRecorder.ReadPublicProperty<string>(startEvent.Value, "Topic"));
                    ...
                    var stopEvent = diagnosticObserver.Records
                        .Where(r => r.Key == Diagnostics.Producer.StopName)
                        .Select(r => new
                        {
                            r.Key,
                            r.Value,
                            DeliveryResult = EventObserverAndRecorder.ReadPublicProperty<DeliveryResult<string, string>>(r.Value, "DeliveryResult")
                        })
                        .Where(r => r.DeliveryResult.Message.Key == "test key 0")
                        .FirstOrDefault();

                    Assert.NotNull(stopEvent);

                    Assert.Equal(partitionedTopic, EventObserverAndRecorder.ReadPublicProperty<string>(stopEvent.Value, "Topic"));
                    Assert.Equal(PersistenceStatus.Persisted, stopEvent.DeliveryResult.Status);
                    Assert.Equal("test val 0", stopEvent.DeliveryResult.Value);

                    string stopTraceId = stopEvent.DeliveryResult.Headers?.Where(h => h.Key == Diagnostics.TraceParentHeaderName).Select(h => Encoding.UTF8.GetString(h.GetValueBytes())).FirstOrDefault();
