    ...
    public static partial class Tests
    {
        ...
        public static void Consumer_Pause_Resume(string bootstrapServers, string singlePartitionTopic, string partitionedTopic)
        {
                    ...
                    .Build())
            {
                ...
                producer.ProduceAsync(singlePartitionTopic, new Message<byte[], byte[]> { Value = Serializers.Utf8.Serialize("test value", null) }).Wait();
                record = consumer.Consume(TimeSpan.FromSeconds(10));
                Assert.NotNull(record?.Message);

                consumer.Pause(assignment);
                producer.ProduceAsync(singlePartitionTopic, new Message<byte[], byte[]> { Value = Serializers.Utf8.Serialize("test value 2", null) }).Wait();
                record = consumer.Consume(TimeSpan.FromSeconds(10));
                Assert.Null(record);
