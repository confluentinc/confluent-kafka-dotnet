...
namespace Confluent.Kafka.IntegrationTests
{
    public partial class Tests
    {
        ...
        private void checkGroupInfo(
            GroupInfo gi, string state, string protocol, string groupID, Dictionary<string, List<TopicPartition>> clientIdToToppars) 
        {
            Assert.Equal(groupID, gi.Group);
            Assert.Equal(ErrorCode.NoError, gi.Error.Code);
            Assert.Equal(state, gi.State);
        ...
        public void AdminClient_ListDescribeConsumerGroupOffsets(string bootstrapServers)
        {
            LogToFile("start AdminClient_ListDescribeConsumerGroupOffsets");
                ...
                BootstrapServers = bootstrapServers }).Build())
            {
                ...
                var groups = adminClient.ListConsumerGroups(TimeSpan.FromSeconds(30));
                ...
                groups = adminClient.ListConsumerGroups(TimeSpan.FromSeconds(30));
                Assert.Single(groups.Where(group => group.Group == groupID));
                Assert.Empty(groups.Where(group => group.Group == nonExistentGroupID));

                groups = adminClient.DescribeConsumerGroups(new List<String>() { groupID }, TimeSpan.FromSeconds(30));
                ...
                checkGroupInfo(group, "Stable", "range", groupID, clientIdToToppars);
                ...
                checkGroupInfo(group, "Empty", "", groupID, clientIdToToppars);
            }

            Assert.Equal(0, Library.HandleCount);
            LogToFile("end   AdminClient_ListDescribeConsumerGroupOffsets");
        }
    }
}
