...
namespace Confluent.Kafka.Serialization
{
    ...
    /// </remarks>
    public class AvroNativeDeserializer<T> : INativeDeserializer<T>
    {
        ...
        private void Initialize()
        {
            ...
            {
                ReaderSchema = (Avro.Schema) typeof(T).GetField("_SCHEMA", BindingFlags.Public | BindingFlags.Static)
                    .GetValue(null);
            }
        ...
        /// </exception>
        public AvroNativeDeserializer(ISchemaRegistryClient schemaRegisteryClient)
        {
        ...
        public unsafe T Deserialize(IntPtr msgBuf, uint msgLen, IntPtr msgTopic)
        {
            ...
            if (msgLen < 5)
                throw new ArgumentException($"Data length not sufficient. expected: >= 5, actual: {msgLen}");
            var ptr = (byte*) msgBuf.ToPointer();
            var magicByte = ptr[0];
            if (magicByte != Constants.MagicByte)
            {
                var mbuf = new byte[(int) msgLen];
                // may change in the future.
                Marshal.Copy(msgBuf, mbuf, 0, (int) msgLen);
                throw new InvalidDataException(
                    $"magic byte should be 0, not {magicByte}. msg='{BitConverter.ToString(mbuf)}'");
            }

            ptr++;
            var writerIdBigEndian = Unsafe.Read<int>(ptr);
            ptr += 4;
            var writerId = IPAddress.NetworkToHostOrder(writerIdBigEndian);

            if (!datumReaderBySchemaId.TryGetValue(writerId, out var datumReader))
            ...
            return datumReader.Read(default(T), new BinaryDecoder(new UnmanagedMemoryStream(ptr, msgLen)));
        }

        /// <include file='../../Confluent.Kafka/include_docs.xml' path='API/Member[@name="IDeserializer_Configure"]/*' />
        public IEnumerable<KeyValuePair<string, object>> Configure(IEnumerable<KeyValuePair<string, object>> config,
            bool isKey)
        {
            ...
            {
                throw new ArgumentException(
                    $"{keyOrValue} AvroDeserializer: unexpected configuration parameter {avroConfig.First().Key}");
            }
            ...
            if (srConfig.Count() != 0)
            {
                ...
                {
                    throw new ArgumentException(
                        $"{keyOrValue} AvroDeserializer schema registry client was configured via both the constructor and configuration parameters.");
                }
            ...
            {
                throw new ArgumentException(
                    $"{keyOrValue} AvroDeserializer schema registry client was not supplied or configured.");
            }
