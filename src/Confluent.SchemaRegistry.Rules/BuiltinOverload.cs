...
namespace Confluent.SchemaRegistry.Rules
{
    public class BuiltinOverload
    {
        ...
        public static Overload[] Create()
        {
            return new[]
            {
                ...
                Overload.NewOverload(
                    ...
                    v => IsCelNull(v)
                        ? NullT.NullValue
                        : Guard(() => VariantOrNull(VariantUtils.ToVariant(v.Value()))),
                    (a, b) => Guard(() =>
                    {
                        byte[] metadata = ToBytes(b, "variant(value, metadata)", "metadata");
                        if (metadata.Length == 0)
                        {
                            throw new ArgumentException(
                                "variant(value, metadata): metadata is empty, so there is no "
                                + "variant to read");
                        }

                        return (IVal)VariantT.Of(new SrVariant(
                            ToBytes(a, "variant(value, metadata)", "value"), metadata));
                    }),
                    null),
        ...
        // An absent variant (VariantUtils.ToVariant returns null) reports as CEL null, which is
        // what every navigation accessor already does for a CEL-null receiver.
        private static IVal VariantOrNull(SrVariant v) =>
            v == null ? (IVal)NullT.NullValue : VariantT.Of(v);
