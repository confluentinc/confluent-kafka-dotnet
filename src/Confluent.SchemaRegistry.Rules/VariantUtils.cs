...
namespace Confluent.SchemaRegistry.Rules
{
    ...
    internal static class VariantUtils
    {
        ...
        /// </summary>
        /// <returns>
        ///     The decoded Variant, or <c>null</c> when the input carries no metadata bytes and
        ///     so holds no variant at all — a protobuf field left unset, or an Avro variant
        ///     record whose byte fields are empty. Callers report that as CEL null; the
        ///     <see cref="SrVariant" /> constructor reads the metadata version byte, so an empty
        ///     buffer has to be caught before construction.
        /// </returns>
        ...
        private static SrVariant FromMessage(IMessage message)
        {
            ...
            {
                // Absent: an unset field. Nothing to read, so the caller reports CEL null.
                return null;
            }
        ...
        private static SrVariant FromMap(IDictionary dict)
        {
            ...
            {
                byte[] metadataBytes = CoerceBytes(metadata);
                // Absent: an Avro variant record with empty byte fields.
                return metadataBytes.Length == 0
                    ? null
                    : new SrVariant(CoerceBytes(value), metadataBytes);
            }
