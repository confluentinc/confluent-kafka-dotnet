...
namespace Confluent.SchemaRegistry
{
    ...
    public sealed class Variant
    {
        ...
        private void WriteJson(StringBuilder sb)
        {
            ...
            switch (t)
            {
                ...
                case VariantType.TimestampTz:
                    sb.Append('"').Append(FormatInstantMicros(GetLong())).Append('"');
                    break;
                case VariantType.TimestampNtz:
                    sb.Append('"').Append(FormatLocalDateTimeMicros(GetLong())).Append('"');
                    break;
        ...
        /// <summary>
        ///     The range a timestamp may occupy when rendered to JSON, in microseconds since the
        ///     epoch: 0001-01-01T00:00:00 through 9999-12-31T23:59:59.999999. That is the
        ///     four-digit-year form every client renders: for the zone-aware types it is RFC 3339 -
        ///     also google.protobuf.Timestamp's range, and the range timestamp(...) enforces when
        ///     constructing a CEL timestamp. The zone-less types carry no offset, so they are
        ///     ISO-8601 local date-times rather than RFC 3339 (which has no zone-less form); they
        ///     share the range so both stay readable by the same date parsers. It is also, as it
        ///     happens, exactly what System.DateTime can represent.
        /// </summary>
        /// <remarks>
        ///     A variant TIMESTAMP_TZ / TIMESTAMP_NTZ is an arbitrary int64 of microseconds -
        ///     roughly +/-292,471 years - so it can hold instants that have no RFC 3339 form.
        ///     Those are refused rather than rendered: ISO-8601's expanded year
        ///     ("+10000-01-01T00:00:00Z") is not RFC 3339 and would not parse back. The
        ///     nanosecond-based types need no check, because an int64 of nanoseconds spans only
        ///     1677-2262, inside this range at both ends.
        /// </remarks>
        private const long MinTimestampMicros = -62135596800000000L;
        private const long MaxTimestampMicros = 253402300799999999L;

        /// <summary>
        ///     The range a DATE may occupy, in days since the epoch: 0001-01-01 through 9999-12-31.
        ///     RFC 3339's full-date requires date-fullyear = 4DIGIT, so an expanded or negative
        ///     year is not a valid full-date. A variant DATE is an int32 of days - roughly
        ///     +/-5.8 million years - so those are reachable and are refused rather than rendered.
        /// </summary>
        private const int MinDateEpochDay = -719162;
        private const int MaxDateEpochDay = 2932896;

        private static long CheckDateRange(long epochDay)
        {
            if (epochDay < MinDateEpochDay || epochDay > MaxDateEpochDay)
            {
                throw new VariantException(string.Format(CultureInfo.InvariantCulture,
                    "date epoch day ({0}) must be in range [{1}, {2}]",
                    epochDay, MinDateEpochDay, MaxDateEpochDay));
            }
        ...
        ///     23:59:59.999999. RFC 3339's partial-time requires time-hour = 2DIGIT in 00-23, so a
        ///     value at or past 24 hours (or negative) has no valid form. Checking also removes an
        ///     overflow, since micros * 1000 wraps for a large enough value.
        /// </summary>
        private const long MinTimeMicros = 0L;
        private const long MaxTimeMicros = 86_400_000_000L - 1L;
        ...
        private static long CheckTimeRange(long micros)
        {
            ...
            {
                throw new VariantException(string.Format(CultureInfo.InvariantCulture,
                    "time microseconds of day ({0}) must be in range [{1}, {2}]",
                    micros, MinTimeMicros, MaxTimeMicros));
            }

            return micros;
        }

        private static void CheckMicrosRange(long micros)
        {
            if (micros < MinTimestampMicros || micros > MaxTimestampMicros)
            {
                throw new VariantException(string.Format(CultureInfo.InvariantCulture,
                    "timestamp microseconds ({0}) must be in range [{1}, {2}]",
                    micros, MinTimestampMicros, MaxTimestampMicros));
            }
        }

        private static string FormatParts(long sec, long nano, bool utc)
        {
            DateTime dt = DateTimeOffset.FromUnixTimeSeconds(sec).UtcDateTime;
            return string.Format(CultureInfo.InvariantCulture,
                "{0:D4}-{1:D2}-{2:D2}T{3:D2}:{4:D2}:{5:D2}{6}{7}",
        ...
        {
            return FormatParts(FloorDiv(totalNanos, 1_000_000_000L),
                FloorMod(totalNanos, 1_000_000_000L), true);
        }

        private static string FormatLocalDateTime(long totalNanos)
        {
            return FormatParts(FloorDiv(totalNanos, 1_000_000_000L),
                FloorMod(totalNanos, 1_000_000_000L), false);
        }
        ...
        // (2262-04-11T23:47:16.854775Z) and wrapped silently to a plausible-looking date in the
        // past - year 10000 rendered as 1816. The whole renderable range cannot be expressed in
        // nanoseconds at all, so the split is required, not merely safer.
        private static string FormatInstantMicros(long micros)
        {
            CheckMicrosRange(micros);
            return FormatParts(FloorDiv(micros, 1_000_000L),
                FloorMod(micros, 1_000_000L) * 1000L, true);
        }
        ...
        private static string FormatLocalDateTimeMicros(long micros)
        {
            CheckMicrosRange(micros);
            return FormatParts(FloorDiv(micros, 1_000_000L),
                FloorMod(micros, 1_000_000L) * 1000L, false);
        }

        private static string FormatLocalTime(long micros)
        {
            long nanoOfDay = CheckTimeRange(micros) * 1000L;
        ...
        {
            DateTime dt = DateTimeOffset.FromUnixTimeSeconds(CheckDateRange(days) * 86400L).UtcDateTime;
            return string.Format(CultureInfo.InvariantCulture,
